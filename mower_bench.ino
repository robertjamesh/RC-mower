...
unsigned long lastUpdateTime    = 0;  // tracks last fresh controller packet
...
void rotateMotorRaw(int idx, int pwm) {
  ledcWrite(idx, pwm); // idx is the channel number (matches pwmPins index)
}
...
void setup() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    ledcSetup(i, 20000, 8);        // channel i, 20kHz, 8-bit
    ledcAttachPin(pwmPins[i], i);  // attach pin to channel
    rotateMotorRaw(i, 0);
  }
...
void loop() {
  ...
  // If no fresh packet for 100ms, zero targets.
  // Short enough to feel responsive, long enough to avoid jerkiness between packets.
  if (millis() - lastUpdateTime > 100) {
    for (int i = 0; i < 4; i++) targetSpeeds[i] = 0;
  }
